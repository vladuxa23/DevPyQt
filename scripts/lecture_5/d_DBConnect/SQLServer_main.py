import sys
from PySide6 import QtWidgets, QtGui, QtCore  # Импорт класса, который содержит элементы окна
import pyodbc
...
class MyFirstWindow(QtWidgets.QWidget):  # Наследуемся от QMainWindow
    ...
    def initDB(self):
        ...
        conn = 'DRIVER={ODBC Driver 18 for SQL Server};SERVER=' + server + ';DATABASE=' + database + ';UID=' + username + ';PWD=' + password + ";Encrypt=yes;TrustServerCertificate=yes"
...
if __name__ == "__main__":
    ...
    app.exec()  # Если exit, то код дальше не исполняется
